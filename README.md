...
The big problem I solved for our school is the current manual progress reporting process for parent interviews, and our lack of a system for tracking how kids are doing academically:
...
* **Rating forgery prevention, verified.** An RLS `with check (auth.uid() = user_id)` policy means a teacher can't submit a rating as another teacher — tested by attempting an insert as a real second user and getting a `42501` policy violation.
* **Chose not to build multi-teacher averaging.** User research showed our school converges on a shared judgment, so last-write-wins already fit the real workflow.
* **Derived state over stored state** — the current rating is computed from the append-only log, not stored.
* **Custom hooks** (`useFetch<T>`, `useRateStudent`) separating data and logic from UI.
...
* AI summaries of student progress
* Student pages with graphs of progress over time
